...
if exist('w','var')&&numel(w)>0 %#ok<*NODEF>
    ...
    if exist_affine_model~=2
    % Check that system is affine in unknown inputs:    
...
if exist('u','var')&& numel(u)>0
    ...
    if exist_affine_model~=2
        ...
        hu=sym(zeros(m,nu));
        % Check that system is affine in known inputs and almacenate coefficients:
        for j=1:nu
            for i=1:ns
                try
        ...
        save(fullaffinename,'x','u','w','p','f','h','fu','fxw','hu','hxw');
    end
else
    u=[];
    nu=0;
...
while k<=opts.affine_kmax && stage_time(k+1)-stage_time(k)<opts.affine_tStage
    ...
    % Update delta_omega (Lie derivative of Omega): 
    Delta_Omega=dif_Delta_omega*[fxw fu]; 
    % Reshape delta_omega as a column vector:
    Delta_Omega=reshape(Delta_Omega,[],1);
    % Include as states only nonzero kth derivatives of unknown inputs:
    ...
    dif_Delta_omega=jacobian(Delta_Omega,xaug);
    % Update the differential of Omega:
...
%================== Update system dynamic functions=======================%
